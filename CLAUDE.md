...
- **Dependabot** (`.github/dependabot.yml`): weekly `uv` and `github-actions` updates, with the `uv` ecosystem's minors and patches grouped into one PR. Its `ignore:` list is what keeps it from proposing the bumps the pins above forbid: `starlette` majors, **`fastapi` minors *and* majors** (Dependabot classifies fastapi's 0.x releases by patch position, so `0.136 → 0.137` is a *minor* — ignoring only majors would let the rate-limit-breaking bump through), plus `chromadb` and `redis` majors, which are shelved pending a client-compatibility pass rather than forbidden. `tests/unit/test_dependabot_config.py` is the guard that keeps those entries present; `docs/dependency-runbook.md` is the process for accepting or shelving anything that does arrive.
...
- **`004-pydantic-ai-v2-unblock` is the single active spec; `003-pydantic-ai-v2-migration` is sealed.** `003` shipped units 1–9 (tasks 1–9, 65 of 90 subtasks) before its task 9 recorded the adapter-compatibility gate **FAILED** (evidence: `docs/adapter-probe-report.md`); its tasks 10–12 (25 subtasks, Requirements 9–11) are closed as superseded by `004` rather than completed — see the dated addenda in `003`'s own `tasks.md`/`spec.md`. `004` re-executes that gate under its own Requirement 4 (task 6) before re-attempting the v2 code migration, its behavioural pinning, and the Redis key-prefix cutover as `004`'s own change units. `004`'s tasks are boundary-scoped (`_Boundary:_` lists the only files a task may touch) and several of them own edits to *this file*, so check `.sdd/specs/004-pydantic-ai-v2-unblock/tasks.md` before making an architectural change here: the change may already be assigned, sequenced, and required to ship behind a failing test.
- **`pydantic-ai-slim` stays pinned to the 1.x line** (`>=1.99.0,<2.0` in `pyproject.toml`) **until `004`'s gate is recorded as passed** — sealing `003` above is not itself authorisation to migrate. Only `004`'s task 7 retires this pin, alongside the constraint bump itself.
