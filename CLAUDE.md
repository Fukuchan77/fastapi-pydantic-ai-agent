...
A second, unrelated group of three `--ignore-vuln` entries covers `chromadb` 0.6.3 (`CVE-2026-45830`, `CVE-2026-45831`, `CVE-2026-45833`). Unlike the starlette group, none of the three has a fix release to upgrade into — PyPI/OSV list `fixed_in: []` for all three, and the affected range covers even the latest chromadb (1.5.9), so lifting the `chromadb<1.0` cap would not close them either. All three require ChromaDB's server (multi-tenant HTTP API, RBAC, `trust_remote_code`), and this codebase only ever constructs the embedded client (`chromadb.PersistentClient`/`chromadb.Client()` in `app/stores/vector_store/chroma.py`) — no `HttpClient`, no tenants, no RBAC config — so they're unreachable today. They stop being dormant the moment any code switches to `chromadb.HttpClient` or this app starts talking to a standalone Chroma server (see the security note next to the `HttpClient` example in `docs/production_deployment.md`); revisit the ignore list before that ships, and re-run `uv run pip-audit` without `--ignore-vuln` after any chromadb bump to check whether upstream has since shipped a fix.
